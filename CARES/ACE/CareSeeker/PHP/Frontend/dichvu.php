...
<head>
    ...
    <style>
        * {
        ...
        .btn-detail:last-child:hover {
            background: #FF6B81
        }

        .btn-detail:first-child:hover {
            background: #E55B70;
        }
...
<body>
    ...
    <script>
        ...
        function createCardHtml(caregiver) {
            const hinh_anh_url = caregiver.hinh_anh && caregiver.hinh_anh !== '' ? caregiver.hinh_anh : 'img/default_avatar.png';
            ...
            return `
                <div class="card" data-name="${caregiver.ho_ten.toLowerCase()}"
                    ...
                    <div class="card-content">
                        ...
                        <div class="info"><i class="fas fa-briefcase"></i> Kinh nghiệm: ${caregiver.kinh_nghiem}</div>
        ...
        async function fetchCaregivers() {
            const caregiverGrid = document.getElementById("caregiverGrid");
            const apiUrl = '../Backend/api_caregiver.php?action=list_all';
            ...
            try {
                ...
                if (!response.ok) {
                    ...
                    caregiverGrid.innerHTML = `<p style="text-align:center; color:red; padding: 50px; grid-column: 1 / -1;">Lỗi kết nối API (${response.status}): Vui lòng kiểm tra trạng thái đăng nhập hoặc đường dẫn. Chi tiết: ${errorText.substring(0, 100)}...</p>`;
                    return;
                }
                ...
                if (result.success && Array.isArray(result.data)) {
                    ...
                    caregiverGrid.innerHTML = `<p style="text-align:center; color:red; padding: 50px; grid-column: 1 / -1;">Lỗi tải dữ liệu: ${result.message || 'API trả về lỗi hoặc không có dữ liệu.'}</p>`;
                }
            } catch (error) {
                console.error('Lỗi gọi API:', error);
                caregiverGrid.innerHTML = `<p style="text-align:center; color:red; padding: 50px; grid-column: 1 / -1;">Lỗi kết nối Microservice. Vui lòng kiểm tra trạng thái server. (${error.message})</p>`;
            }
        }
