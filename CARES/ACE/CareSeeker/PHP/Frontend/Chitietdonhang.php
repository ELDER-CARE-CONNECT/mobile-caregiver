...
// 1. Kiểm tra Session ID Khách Hàng
if (!isset($_SESSION['id_khach_hang'])) {
    header("Location: ../../../Admin/frontend/auth/login.php");
    exit();
}

// Lấy ID từ tham số 'id'
$id_don_hang = isset($_GET['id']) ? intval($_GET['id']) : 0;

// 2. Xử lý lỗi chuyển hướng từ VNPAY (URL chứa id=0)
if ($id_don_hang === 0 && isset($_GET['order_id_original'])) {
    $id_don_hang_original = intval($_GET['order_id_original']);
    if ($id_don_hang_original > 0) {
        $id_don_hang = $id_don_hang_original;
        $payment_status = isset($_GET['payment']) ? "&payment=" . $_GET['payment'] : '';
        header("Location: chitietdonhang.php?id=$id_don_hang$payment_status");
        exit();
    }
}
...
// Định nghĩa biến cho JavaScript
...
<head>
    ...
    <style>
        * { box-sizing: border-box; margin: 0; padding: 0; font-family: 'Inter', sans-serif; }
        ...
        .status-dang_hoan_thanh, .status-dang_tien_hanh { background: #e3f2fd; color: #2196f3; }
        ...
        .btn-home { background-color: #FF6B81; }
        ...
        .modal-overlay { position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.6); display: flex; justify-content: center; align-items: center; z-index: 1000; opacity: 0; transition: opacity 0.3s ease; visibility: hidden; }
        .modal-overlay:not(.hidden) { opacity: 1; visibility: visible; }
        .success-modal { background: #fff; padding: 40px 30px; border-radius: 16px; text-align: center; width: 90%; max-width: 450px; box-shadow: 0 10px 30px rgba(0, 0, 0, 0.2); transform: scale(0.9); transition: transform 0.3s ease; }
        .modal-overlay:not(.hidden) .success-modal { transform: scale(1); }
        .success-icon { font-size: 72px; color: #4caf50; margin-bottom: 20px; animation: popIn 0.5s cubic-bezier(0.68, -0.55, 0.27, 1.55) 0.2s forwards; transform: scale(0); }
        .success-modal h2 { font-size: 24px; font-weight: 700; color: #333; margin-bottom: 10px; }
        .success-modal p { font-size: 16px; color: #666; margin-bottom: 30px; line-height: 1.5; }
        .success-modal .button { width: 100%; justify-content: center; }
...
<body>
    ...
    <div class="container" id="mainContainer">
        ...
        <div id="contentView" class="hidden">
            ...
            <div class="section-box">
                ...
                <div id="serviceListContainer" style="margin-top: 15px;"></div>
            </div>
            ...
            <div class="buttons" id="actionButtonsContainer"></div>
        </div>
    </div>
    ...
    <script>
        ...
        async function loadData() {
            try {
                const API_BASE = '../Backend/api_gateway.php';
                const routeQuery = `route=order/details&action=get_details&id=${ID_DON_HANG}`;
                
                const response = await fetch(`${API_BASE}?${routeQuery}`, {
                    credentials: 'include'
                });
