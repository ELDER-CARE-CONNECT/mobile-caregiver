    ...
    <script>
        ...
        function renderData(data) {
            ...
            if (services.length > 0) {
                ...
                const status_raw = order.trang_thai_nhiem_vu || 'chua_hoan_thanh';
                const status_list = status_raw.split(';');

                services.forEach((service, i) => {
                    const actual_status = status_list[i] || (statusClass === 'da_hoan_thanh' ? 'hoan_thanh' : 'chua_hoan_thanh');
                    const task_status_class = statusToClass(actual_status);
                    const display_status = (actual_status === 'hoan_thanh') ? 'Hoàn thành ✅' : 'Chờ thực hiện ⏳';

                    serviceContainer.innerHTML += `
                        <div class="service-item">
                            <strong>${service.ten_nhiem_vu}</strong>
                            <span class="task-status status-${task_status_class}">
                                ${display_status}
                            </span>
                        </div>
                    `;
