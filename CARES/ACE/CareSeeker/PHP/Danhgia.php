...
// Lấy ID người chăm sóc (để biết đánh giá ai)
$id_cham_soc = isset($_GET['id']) ? intval($_GET['id']) : 0;
// Lấy ID đơn hàng (để giữ trên URL nếu cần)
$id_don_hang = isset($_GET['order_id']) ? intval($_GET['order_id']) : 0;
    ...
    if ($id_khach_hang_session == 0) {
        ...
        if ($so_sao >= 1 && $so_sao <= 5) {
            try {
                ...
                $id_danh_gia_moi = $pdo->lastInsertId();

                // Cập nhật ID đánh giá này vào các đơn hàng LIÊN QUAN
                $stmt_update_order = $pdo->prepare("
                    UPDATE don_hang 
                    SET id_danh_gia = :id_dg 
                    WHERE id_khach_hang = :id_kh 
                      AND id_cham_soc = :id_cs 
                      AND trang_thai = 'Hoàn thành' 
                      AND (id_danh_gia IS NULL OR id_danh_gia = 0)
                ");
                $stmt_update_order->execute([
                    'id_dg' => $id_danh_gia_moi,
                    'id_kh' => $id_khach_hang_session,
                    'id_cs' => $id_cham_soc
                ]);

                // ==========================================================
                // THAY ĐỔI THEO YÊU CẦU: Chuyển về index.php
                // ==========================================================
...
if ($id_khach_hang_session == 0 && empty($message)) {
    ...
    $caregiver = null;
} elseif ($id_cham_soc == 0 || !$caregiver && empty($message)) {
...
<body>
    <div class="container">
        ...
        <?php if ($caregiver): // Chỉ hiển thị form nếu có thông tin caregiver ?>
        ...
        <div class="section-box">
            ...
            <form action="Danhgia.php?id=<?php echo $id_cham_soc; ?>&order_id=<?php echo $id_don_hang; ?>" method="POST">
