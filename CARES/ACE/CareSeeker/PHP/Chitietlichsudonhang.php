...
if (isset($_SESSION['role']) && $_SESSION['role'] === 'khach_hang' && isset($_SESSION['so_dien_thoai'])) {
    ...
    if ($user) {
        ...
        // Lấy đơn hàng mới nhất có trạng thái "Đã hoàn thành"
        $stmt = $pdo->prepare("
            SELECT dh.*, kh.ten_khach_hang, kh.so_dien_thoai, kh.dia_chi,
                   ncs.ho_ten AS ten_cham_soc, ncs.id_cham_soc
            FROM don_hang dh
            ...
            WHERE dh.id_khach_hang = :id_kh AND LOWER(dh.trang_thai) = 'đã hoàn thành'
...
<body>
    <div class="container">
        ...
        <?php if (isset($_SESSION['role']) && $_SESSION['role'] === 'khach_hang' && isset($_SESSION['so_dien_thoai'])): ?>
            ...
            <?php endif; ?>
        <?php else: ?>
            <div style="color: #d32f2f; text-align: center; margin-top: 20px;">Bạn cần đăng nhập để xem chi tiết lịch sử đơn hàng.</div>
        <?php endif; ?>
    </div>
</body>
</html>