...
try {
    if ($id > 0) {
        ...
        $stmt = $conn->prepare("SELECT *, tong_tien_kiem_duoc AS gia_tien FROM nguoi_cham_soc WHERE id_cham_soc=?");
        ...
        $sql = "
            SELECT ncs.*, COALESCE(AVG(dg.so_sao),0) AS danh_gia_tb, ncs.tong_tien_kiem_duoc AS gia_tien
        ...
        while ($row = $res->fetch_assoc()) {
            ...
            $row['reviews'] = $reviews;
            $row['hinh_anh'] = $row['hinh_anh'] ?? '';
            $row['gia_tien'] = $row['gia_tien'] ?? 0; // thêm giá tiền từ tong_tien_kiem_duoc
            $caregivers[] = $row;
        }
