...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($result->num_rows == 0) {
        ...
        $row['tong_tien_kiem_duoc'] = $row['tong_tien_kiem_duoc'] ?? 0;
        $response = ['success' => true, 'data' => $row];
    }
    $stmt->close();

} elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Cập nhật người chăm sóc
    ...
    $tong_tien_kiem_duoc = intval($_POST['tong_tien_kiem_duoc'] ?? 0);
    ...
    if (!empty($_FILES['hinh_anh']['name'])) {
        ...
        if ($_FILES["hinh_anh"]["size"] > 2 * 1024 * 1024) {
        ...
        if (move_uploaded_file($_FILES["hinh_anh"]["tmp_name"], $target_file)) {
            if (!empty($hinh_anh) && file_exists("../../" . $hinh_anh)) unlink("../../" . $hinh_anh);
    ...
    // Cập nhật DB
    $stmt = $conn->prepare("UPDATE nguoi_cham_soc 
        SET ho_ten=?, dia_chi=?, tuoi=?, gioi_tinh=?, chieu_cao=?, can_nang=?, kinh_nghiem=?, hinh_anh=?, tong_tien_kiem_duoc=? 
        WHERE id_cham_soc=?");
    $stmt->bind_param("ssississii", $ho_ten, $dia_chi, $tuoi, $gioi_tinh, $chieu_cao, $can_nang, $kinh_nghiem, $hinh_anh, $tong_tien_kiem_duoc, $id);
