...
<main class="main-content">
    ...
    <form id="editForm" enctype="multipart/form-data">
        ...
        <input type="number" step="0.1" name="chieu_cao" id="chieu_cao">
        <label>Cân nặng (kg):</label>
        <input type="number" step="0.1" name="can_nang" id="can_nang">
        <label>Kinh nghiệm:</label>
        <input type="text" name="kinh_nghiem" id="kinh_nghiem">
...
function loadCaregiver(id){
    $.getJSON(`../../backend/user/nguoi_cham_soc.php`, { id }, function(res){
        if(!res.success){  // Sửa: backend trả success (boolean)
        ...
        // Hiển thị ảnh nếu có, thêm anti-cache
        if(d.hinh_anh && d.hinh_anh.trim() !== ""){
            $('#currentImg').attr('src', `../../${d.hinh_anh}?t=${Date.now()}`).show();  // Thêm ?t= để tránh cache
        } else {
            $('#currentImg').hide();
        }
    }).fail(function(xhr){
...
$('#editForm').submit(function(e){
    ...
    $.ajax({
        ...
        success: function(res){
            $('#loading').hide();
            $('#message').text(res.message).css('color', res.success ? 'green' : 'red');  // Sửa: check res.success
            if(res.success){
